...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //  RENDERING FUNCTIONS
    // ---------------------------------
    const getFilteredTrades = () => { let filtered = [...trades]; if (currentFilters.ticker) filtered = filtered.filter(t => t.ticker.toUpperCase().includes(currentFilters.ticker.toUpperCase())); if (currentFilters.startDate) filtered = filtered.filter(t => t.date >= currentFilters.startDate); if (currentFilters.endDate) filtered = filtered.filter(t => t.date <= currentFilters.endDate); if (currentFilters.plStatus === 'profitable') { filtered = filtered.filter(t => calculateTotalPL(t) !== null && calculateTotalPL(t) > 0); } else if (currentFilters.plStatus === 'losing') { filtered = filtered.filter(t => calculateTotalPL(t) !== null && calculateTotalPL(t) < 0); } filtered.sort((a, b) => { if ($('show-open-trades-toggle').checked) { const aIsOpen = !a.exit, bIsOpen = !b.exit; if (aIsOpen && !bIsOpen) return -1; if (!aIsOpen && bIsOpen) return 1; } return currentFilters.sort === 'date-asc' ? new Date(a.date) - new Date(b.date) : new Date(b.date) - new Date(a.date); }); return filtered; };
    ...
    filterTradesBtn.addEventListener('click', () => { filterTradesModal.innerHTML = `<div class="card p-8 w-full max-w-lg"><h3 class="text-xl font-semibold mb-6 text-white">סינון ומיון עסקאות</h3><form id="filter-trades-form" class="space-y-4"><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label class="block text-sm font-medium mb-1">טיקר</label><input type="text" id="filter-ticker-input" placeholder="הכל" class="input-field w-full" value="${currentFilters.ticker}" oninput="this.value = this.value.toUpperCase()"></div><div><label class="block text-sm font-medium mb-1">מיין לפי</label><select id="filter-sort-select" class="input-field w-full"><option value="date-desc">תאריך (חדש לישן)</option><option value="date-asc">תאריך (ישן לחדש)</option></select></div><div><label class="block text-sm font-medium mb-1">מתאריך</label><input type="date" id="filter-start-date" class="input-field w-full" value="${currentFilters.startDate}"></div><div><label class="block text-sm font-medium mb-1">עד תאריך</label><input type="date" id="filter-end-date" class="input-field w-full" value="${currentFilters.endDate}"></div><div class="md:col-span-2"><label class="block text-sm font-medium mb-1">סטטוס עסקה</label><div class="grid grid-cols-2 gap-2"><button type="button" id="filter-profitable-btn" class="btn-secondary p-2 rounded-lg transition-all duration-200">מרוויחות</button><button type="button" id="filter-losing-btn" class="btn-secondary p-2 rounded-lg transition-all duration-200">מפסידות</button></div></div></div><div class="flex justify-end items-center gap-4 mt-6"><div class="flex gap-4"><button type="button" id="cancel-filter-btn" class="btn-secondary px-6 py-2">ביטול</button><button type="submit" class="btn-primary px-6 py-2">החל</button></div></div></form></div>`; filterTradesModal.classList.remove('hidden'); let selectedPlStatus = currentFilters.plStatus; const profitableBtn = $('filter-profitable-btn'); const losingBtn = $('filter-losing-btn'); const updatePlButtons = () => { profitableBtn.classList.toggle('pl-filter-btn-active', selectedPlStatus === 'profitable'); losingBtn.classList.toggle('pl-filter-btn-active', selectedPlStatus === 'losing'); }; profitableBtn.onclick = () => { selectedPlStatus = selectedPlStatus === 'profitable' ? 'all' : 'profitable'; updatePlButtons(); }; losingBtn.onclick = () => { selectedPlStatus = selectedPlStatus === 'losing' ? 'all' : 'losing'; updatePlButtons(); }; updatePlButtons(); $('filter-sort-select').value = currentFilters.sort; $('cancel-filter-btn').onclick = () => filterTradesModal.classList.add('hidden'); $('filter-trades-form').onsubmit = (e) => { e.preventDefault(); currentFilters = { ticker: $('filter-ticker-input').value, startDate: $('filter-start-date').value, endDate: $('filter-end-date').value, sort: $('filter-sort-select').value, plStatus: selectedPlStatus }; renderTrades(); updateClearFilterButtonVisibility(); filterTradesModal.classList.add('hidden'); }; });
